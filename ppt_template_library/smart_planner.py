...
def build_deck_plan(
    ...
    fallback_layouts = ['data_analysis_slide', 'chart_slide', 'comparison_slide', 'table_slide', 'timeline_slide', 'process_slide', 'gallery_slide', 'map_slide', 'kpi_slide', 'dashboard_slide']
    ...
    for index in range(content_slots):
        ...
        if pages and layout_type == pages[-1]['layout_type']:
            pool = [lt for lt in (style_layouts + data_layouts + fallback_layouts) if lt != layout_type]
            if pool:
                layout_type = pool[index % len(pool)]
